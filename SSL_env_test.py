...
class SSL_env :
    def __init__(self, nsamples=1024, nMic=8) :
        ...
        self.rt = rt                       # Reverberation time (s)
        ...
        self.Sdim_array = d['Sdim_array']
        self.pos_array = d['pos_array']
        self.obstacle_list = d['obstacle_list']
        self.obstacle_room_list = d['obstacle_room_list']
    ...
    def generate_obstacles(self, num_obstacles = 2) :
        ...
        return obstacles, self.obstacle_room


    def acting(self, action, moving_distance) :
        pos_old = self.pos.copy()
        if action == 0:
            self.pos[0] -= 0.5
            self.pos[1] -= 0.5
            moving_distance += 0.5 * np.sqrt(2)
        if action == 1 :
            self.pos[0] -= 0.5
            moving_distance += 0.5
        if action == 2 :
        ...
        if self.reward_clip :
            return act_reward, terminal, moving_distance
        else :
            return act_reward, terminal, moving_distance
    ...
    def step(self, action, step, episode, moving_distance, render=False, test=False, MAX_STEP=128, name='proposed') :
        terminal = False
        pos_old = self.pos.copy()
        reward, terminal, moving_distance = self.acting(action, moving_distance)
        #print(self.pos)
        ...
        for i in range(self.nMic):
            ...
            current_frame = AWGN(current_frame, noise_dB)
        ...
        if terminal :
            if test :
                plt.savefig("./result_test/" + name + "_episode_" + str(episode) + ".png")
            else :
        ...
        return state, distance_reward + reward, terminal, {'pos' : self.pos, 'distance' : distance}, moving_distance

    def reset(self,  episode, render=False) :
        self.Sdim = self.Sdim_array[:,episode]
        self.pos = self.pos_array[:,episode]
        self.roomgrid = np.zeros((self.Rdim[0] * 10, self.Rdim[1] * 10))  # room grid
        self.distance_old = np.linalg.norm(self.pos-self.Sdim, 2)
        self.obstacles = self.obstacle_list[episode]
        self.obstacle_room = self.obstacle_room_list[episode]
        ...
        return state, np.linalg.norm(self.pos-self.Sdim, 2)

    def close(self) :
        print("clear env")

class SSL_env_for_srp_phat :
    def __init__(self, nsamples=1024, nMic=8) :
        self.n_obstacles = 2
        self.T = 26                         # Temperature (C)
        self.c = 331.3 + 0.606 * self.T     # Sound velocity (m/s)
    ...
    def step(self, action, step, episode, moving_distance, render=False, test=False, MAX_STEP=128, name='proposed') :
        ...
        Y_max = np.where(Y==Y.max(), 100, 0)
        Y_max = np.reshape(Y_max, [30, 30])
        Y_max = np.transpose(Y_max)
        action = self.srp_phat_action(Y_max)
        ...
        return state, distance_reward + reward, terminal, {'pos' : self.pos, 'distance' : distance}, moving_distance, action
    ...
    def reset(self,  episode, render=False) :
        ...
        return state, np.linalg.norm(self.pos-self.Sdim, 2), action
...
if __name__ == "__main__" :
    ssl_env = SSL_env()
    for i in range(10):
        ssl_env.step(1, 1,0, render=True)
